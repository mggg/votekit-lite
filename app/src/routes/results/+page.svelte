...
<div class="grid grid-cols-1 gap-6 lg:grid-cols-4">
	<aside
		class="rounded-2xl border border-slate-200/70 bg-white/70 p-3 shadow-sm backdrop-blur lg:col-span-1"
	>
		<div class="mb-3 flex items-center justify-between">
			<h2 class="text-sm font-semibold text-slate-800">My runs</h2>
			<div class="flex gap-1">
				<button
					class="rounded-full px-2 py-1 text-xs text-slate-600 hover:bg-white/70 hover:ring-1 hover:ring-slate-200"
		...
		<div class="space-y-1">
			...
			{:else}
				{#each runs as run}
					<button
						class="relative w-full rounded-xl px-2 py-2 pl-3 text-left text-sm transition hover:bg-white/70 hover:ring-1 hover:ring-slate-200 {selectedIds.includes(
							run.params.id
						)
					...
					>
						{#if selectedIds.includes(run.params.id)}
							<span
								...
								style={`background-color:${getColorHexForId(run.params.id)}; width:8px; height:8px; border-radius:9999px`}
							></span>
						{/if}
						<div class="truncate">{run.params.name}</div>
						<div class="text-xs text-slate-500">
							{new Date(run.params.createdAt).toLocaleString()}
						</div>
					</button>
				{/each}
			{/if}
	...
	<section class="space-y-6 lg:col-span-3">
			...
			{:else}
				<div class="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3">
					{#each selectedRuns as selected}
						<div
							class="rounded-xl border border-slate-200/70 bg-white/70 p-4"
							style={`border-top:4px solid ${getColorHexForId(selected.params.id)}`}
						>
							<h3 class="mb-2 truncate text-sm font-medium text-slate-700">
								{selected.params.name}
							</h3>
							<div
								class="grid grid-cols-6 items-end gap-2 rounded-xl border border-slate-200/70 bg-gradient-to-b from-indigo-50/60 to-white p-3"
							>
								{#each selected.result.slateAElected as count, seats}
									<div class="flex flex-col items-center gap-1">
										<div
											class="w-full rounded-lg shadow-sm"
											style={`background-color:${getColorHexForId(selected.params.id)}; opacity:0.85; height:${count === 0 ? 4 : Math.min(160, 10 + count)}px`}
										></div>
		...
		<div
			class="rounded-2xl border border-slate-200/70 bg-white/70 p-5 text-sm text-slate-600 backdrop-blur"
		>
