	...
	<aside class="card bg-base-100 p-4 shadow-sm lg:col-span-1">
		<div class="mb-3 flex items-center justify-between">
			...
			<div class="flex gap-1">
				<button class="btn btn-soft btn-xs" on:click={selectAll}>Select all</button>
				<button class="btn btn-soft btn-xs" on:click={clearSelection}>Clear</button>
			</div>
		</div>
		<div class="space-y-1">
			{#if runs.length === 0}
			...
			{:else}
				{#each runs as run}
					<button
						class="btn relative w-full justify-start btn-soft {selectedIds.includes(run.params.id)
							? 'btn-primary'
							: 'btn-secondary'}"
			...
			{:else}
				<div class="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3">
					{#each selectedRuns as selected}
						<div
							class="card bg-base-100 p-4 shadow-sm"
