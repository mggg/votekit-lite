<script lang="ts">
	...
	async function submitMock() {
		...
		const votekitConfig: VotekitConfig = {
			id,
			name,
			voterBlocs: {
				blocA: {
					...
					cohesion: {
						slateA: blocACohesion,
						slateB: blocBCohesion
					}
				},
				blocB: {
					count: blocBCount,
	...
	<div class="space-y-4 lg:col-span-2">
		...
		<div class="rounded-2xl border border-slate-200/70 bg-white/70 p-3 shadow-sm backdrop-blur">
			...
			<div class="grid grid-cols-1 gap-3 md:grid-cols-3">
				<button
					type="button"
					class="rounded-xl border border-slate-200/70 bg-white/60 p-3 text-left text-sm hover:bg-white/80 focus:outline-none {ballotGenerator ===
					'sPL'
				...
				>
					...
					<div class="mt-1 text-xs text-slate-600">
						Makes quick picks with minimal deliberation.
					</div>
				</button>
				<button
					type="button"
				...
				>
					...
					<div class="mt-1 text-xs text-slate-600">
						Carefully ranks candidates based on bloc preferences.
					</div>
				</button>
				<button
					type="button"
				...
				>
					...
					<div class="mt-1 text-xs text-slate-600">
						Follows patterns similar to Cambridge ballots.
					</div>
				</button>
			</div>
		</div>
	...
	<div class="space-y-3">
		<div class="rounded-2xl border border-slate-200/70 bg-white/70 p-3 shadow-sm backdrop-blur">
			...
			<label class="mt-2 flex items-center gap-2 text-sm">
				<div>
					<Recaptcha siteKey={PUBLIC_CAPTCHA_KEY} onVerify={handleVerify} />
					<button
		...
		<div
			class="rounded-2xl border border-slate-200/70 bg-white/70 p-3 text-sm text-slate-600 backdrop-blur"
		>
			<p>Preview:</p>
			<ul class="mt-2 list-disc space-y-1 pl-5">
				<li>
					Blocs: A {blocACount} voters (A→A {blocAPrefForA}, A→B {blocAPrefForB}), B {blocBCount} voters
					(B→B {blocBPrefForB}, B→A {blocBPrefForA})
				</li>
				<li>Slates: A {slateANum}, B {slateBNum}</li>
				<li>
					Mode: {mode === 'plurality' ? 'Bloc plurality' : `STV, ${stvNumSeats} seats`}; Behavior: {ballotGenerator}
				</li>
				<li>Trials: {trials}</li>
			</ul>
		</div>
